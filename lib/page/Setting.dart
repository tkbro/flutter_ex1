...
class SettingState extends State<SettingPage> {
  int selectedIndex = 0;
  Color bannerColor = Color.fromRGBO(255, 208, 90, 1);
  final List<String> menuStringList = const [
    '내 찜 모임',
    '최근 본 모임',
    '프리미엄 모임',
    '클래스 만들기',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: buildBottomNavigationBar(selectedIndex),
    );
  }
  ...
  Widget buildMenuListPanel() {
    return Flexible(
      child: Container(
        ...
        child: ListView.builder(
          shrinkWrap: true,
          itemCount: menuStringList.length,
          itemBuilder: ((context, index) {
            return Column(
              children: [
                ListTile(
                  title: Text(menuStringList[index], style: const TextStyle(fontSize: 20)),
